 ...
 * <p>
 * * @see <a href="https://github.com/kang-hyungu/di-study/blob/master/src/test/kotlin/ioc/BeanScopes.kt">원문 코드</a>
 */
@DisplayName("Bean 스코프")
class BeanScopes {
    ApplicationContext applicationContext = new AnnotationConfigApplicationContext(BeanScopesConfig.class);
     ...
     * 클래스 로더 : 자바 클래스를 자바 가상 머신으로 동적 로드하는 자바 런타임 환경의 일부
     * //TODO: 정확히 어떤 의미인지? - 코드단에서 직접 INSTANCE == NULL 같이 코딩해야한다는 뜻?
     * 스프링은 컨테이너에서 특정 클래스에 대해 하나의 Bean을 정의하면 컨테이너는 해당 빈 정의에 의해 정의된 클래스의 인스턴스를 하나만 생성합니다.
     * """
     */
    @Test
    void 싱글톤_스코프() {
        final var firstSingletonObject = applicationContext.getBean("singletonBean", SampleObject.class);
        final var secondSingletonObject = applicationContext.getBean("singletonBean", SampleObject.class);
        assertThat(firstSingletonObject).isSameAs(secondSingletonObject);
    }
    ...
    @Test
    void 프로토타입_스코프() {
        final var firstSingletonObject = applicationContext.getBean("prototypeBean", SampleObject.class);
        final var secondSingletonObject = applicationContext.getBean("prototypeBean", SampleObject.class);

        assertThat(firstSingletonObject).isNotSameAs(secondSingletonObject);
    }
     ...
     */
    @Nested
    @DisplayName("프로토타입_의존성")
    class ProtoTypeScopeCase {
        /**
         * """
        ...
        @Test
        void 프로토타입_싱글톤에_주입() {
            final var firstPrototypeIntoSingleton =
                    applicationContext.getBean("prototypeIntoSingleton", PrototypeIntoSingleton.class);
            final var secondPrototypeIntoSingleton =
                    applicationContext.getBean("prototypeIntoSingleton", PrototypeIntoSingleton.class);

            // ❓프로토타입 Bean인데 왜 같은 인스턴스를 반환할까요?
            // 싱글톤이므로, 애초에 같은 prototypeIntoSingleton 이고, 처음 주입될 때 값은 객체 주입
            assertThat(firstPrototypeIntoSingleton).isSameAs(secondPrototypeIntoSingleton);
            assertThat(firstPrototypeIntoSingleton.getPrototypeBean()).isSameAs(secondPrototypeIntoSingleton.getPrototypeBean());
        }
        ...
        void 싱글톤_프로토타입에_주입() {
            final var firstPrototypeIntoSingleton =
                    applicationContext.getBean("singletonIntoPrototype", SingletonIntoPrototype.class);
            final var secondPrototypeIntoSingleton =
                    applicationContext.getBean("singletonIntoPrototype", SingletonIntoPrototype.class);

            // 둘 다 프로토타입 Bean이므로 서로 다른 인스턴스를 반환합니다.
            assertThat(firstPrototypeIntoSingleton).isNotSameAs(secondPrototypeIntoSingleton);

            // 프로토타입 Bean이 의존하는 싱글톤 Bean은 동일한 인스턴스를 사용합니다.
            assertThat(firstPrototypeIntoSingleton.getSingletonBean()).isSameAs(secondPrototypeIntoSingleton.getSingletonBean());
        }
    }
