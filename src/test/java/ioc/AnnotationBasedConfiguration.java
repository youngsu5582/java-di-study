 ...
 *
 * @see <a href="https://github.com/kang-hyungu/di-study/blob/master/src/test/kotlin/ioc/AnnotationBasedConfiguration.kt">원문 코드</a>
 */
@DisplayName("어노테이션 기반 컨테이너 구성(Annotation-based Container Configuration)")
class AnnotationBasedConfiguration {
    private final ApplicationContext applicationContext = new AnnotationConfigApplicationContext(AnnotationConfig.class);
    ...
    @Test
    void Inject_사용한_주입() {
        final var movieRecommender = applicationContext.getBean(MovieRecommender.class);
        final var customerPreferenceDao = movieRecommender.getCustomerPreferenceDao();

        assertThat(customerPreferenceDao).isNotNull();
    }
    ...
    void 클래스_컬렉션에_자동주입() {
        final var movieRecommender = applicationContext.getBean(MovieRecommender.class);
        final var movieCatalogs = movieRecommender.getMovieCatalogs();
        assertThat(movieCatalogs).containsKey("movieCatalog");
    }
    ...
    void 인터페이스_컬렉션에_자동주입() {
        ...
        //TODO : MessageService 만 AutoWired 한 이유는? - Inject 로 변경해도 동작 확인
        final var messageService = applicationContext.getBean(MessageService.class);

        messageService.println();

        // Bean의 이름이 key, Bean의 구현체가 value로 주입됩니다.
        final var kakaotalkMessageSender = messageService.getMessageSender("kakaotalkMessageSender");

        // 생성자에 인터페이스만 명시했는데, 스프링이 자동으로 Bean 구현체를 찾아서 주입했습니다.
        assertThat(kakaotalkMessageSender).isInstanceOf(MessageSender.class);
    }
    ...
    @Test
    void 선택적_의존() {
        final var simpleMovieLister = applicationContext.getBean(SimpleMovieLister.class);

        // NoImplements는 인터페이스이고 구현체가 없습니다.
        // 하지만 setNoImplements() 메서드에 @Autowired(required = false)를 설정했기 때문에 예외가 발생하지 않습니다.
        // true 라면 UnsatisfiedDependencyException 발생
        final var noImplements = simpleMovieLister.getNoImplements();

        assertThat(noImplements).isNull();
    }
    ...
    void 우선순위_지정() {
        ...
        // Primary 주석일시, UnsatisfiedDependencyException 발생
        final var applicationContext = new AnnotationConfigApplicationContext(PrimaryConfig.class);
        final var primaryConfig = applicationContext.getBean(PrimaryConfig.class);
        final var movieCatalog = primaryConfig.getMovieCatalog();

        assertThat(movieCatalog).isInstanceOf(MovieCatalog.class);
    }
    ...
    void 의존주입_지정() {
        ...
        // Qualifier를 제거하면, UnsatisfiedDependencyException 발생
        final var applicationContext = new AnnotationConfigApplicationContext(QualifierConfig.class);
        final var qualifierConfig = applicationContext.getBean(QualifierConfig.class);
        final var movieCatalog = qualifierConfig.getMovieCatalog();
    ...
    @Test
    void 프로퍼티_값_주입() {
        final var applicationContext = new AnnotationConfigApplicationContext(PropertyConfig.class);
        final var sampleValue = applicationContext.getBean(SampleValue.class);

        assertThat(sampleValue.getValue()).isEqualTo("외부값");

        // defaultValue는 외부 프로퍼티에 정의되어 있지 않습니다.
        // @Value("${value.default:기본값}")에서 설정한 문자열 "기본값"이 주입됩니다.
        assertThat(sampleValue.getDefaultValue()).isEqualTo("기본값");
    }
}
